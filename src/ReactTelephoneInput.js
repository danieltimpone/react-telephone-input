...
export var ReactTelephoneInput = createReactClass({
  ...
  guessSelectedCountry: memoize(function(inputNumber) {
    var secondBestGuess =
      findWhere(allCountries, { iso2: this.props.defaultCountry }) || this.props.onlyCountries[0];
  ...
  searchCountry() {
    ...
    const probableCandidateIndex =
      findIndex(this.props.onlyCountries, probableCandidate) + this.state.preferredCountries.length;
    ...
    switch (event.which) {
      ...
      case keys.ENTER:
        console.log(
          ...
          this.props.onlyCountries[this.state.highlightCountryIndex]
        );
